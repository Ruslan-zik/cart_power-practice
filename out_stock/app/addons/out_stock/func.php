...
function fn_out_stock_get_products(&$params, $fields, $sortings, &$condition, &$join, $sorting, $group_by, $lang_code, $having)
{
    if (!empty(Tygh::$app['session']['auth']['user_id']) && $params['area'] == 'C') {
        $out_of_stock = db_get_field("SELECT out_of_stock FROM ?:users WHERE user_id = ?i", Tygh::$app['session']['auth']['user_id']);

        if (Registry::get('settings.General.show_out_of_stock_products') != 'N' && $out_of_stock == 'N') {
            $condition .= db_quote(
