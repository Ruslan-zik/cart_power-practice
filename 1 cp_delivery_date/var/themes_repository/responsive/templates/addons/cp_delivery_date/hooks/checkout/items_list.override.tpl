                        ...
                        <td class="ty-cart-content__product-elem ty-cart-content__description" style="width: 50%;">
                            ...
                            {/if}
                                <div class="ty-cp_delivery_date ty-reward-group">
                                    <label class="ty-control-group__label ty-product-options__item-label" for="cp_delivery_date_{$obj_id}">{(__("cp_delivery_date.cp_delivery_date"))}:</label>
                                    <div class="ty-control-group__item">
                                    {if $product.extra.cp_delivery_date}
                                        {assign var="date_val" value=$product.extra.cp_delivery_date}
                                    {else}
                                        {assign var="date_val" value=$smarty.const.TIME}
                                    {/if}
                                    {include file="common/calendar.tpl" date_id="cp_delivery_date_{$obj_id}" date_name="cart_products[$obj_id][extra][cp_delivery_date]" date_val=$date_val start_year=$smarty.now|date_format:"%Y"}
                                    </div>
                                </div>
