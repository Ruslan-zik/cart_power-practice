...
namespace Tygh\Addons\cp_delivery_date\Template\Snippet\Table;
 ...
 *
 * @package Tygh\Addons\cp_delivery_date\Template\Snippet\Table
 */
class ProductVariable implements IVariable, IActiveVariable
{
    ...
    public function __construct(ItemContext $context, array $config, Core $view, Formatter $formatter)
    {
        ...
        foreach ($this->product as $field => $item) {
            if (property_exists($this, $field) && !in_array($field, ['image', 'main_pair', 'product', 'view', 'context', 'options'])) {
                $this->{$field} = $item;
            }
        }
        ...
        if (!empty($this->product['cp_delivery_date'])) {
            $this->cp_delivery_date = fn_date_format($this->product['cp_delivery_date']);
        }
    ...
    public static function attributes()
    {
        return array(
            ...
            'is_edp', 'edp_shipping', 'stored_price', 'counter', 'main_pair', 'image', 'options', 'cp_delivery_date',
            'raw' => array(
